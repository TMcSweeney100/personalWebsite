export const profile = {
  heroKicker: "Recruiter-friendly one-page portfolio (Placeholder)",
  name: "Your Name",
  role: "Software Engineer",
  valueProposition:
    "I build reliable, user-friendly software and ship features with a focus on clarity, performance, and maintainability. (Placeholder)",
  highlights: [
    "Highlight #1 (Placeholder)",
    "Highlight #2 (Placeholder)",
    "Highlight #3 (Placeholder)",
    "Highlight #4 (Placeholder)",
  ],
  snapshot: {
    title: "Quick snapshot (Placeholder)",
    description:
      "Use this box for a 30-second overview: domains, strengths, and what roles you're targeting. (Placeholder)",
    bullets: [
      "Focus area / domain #1 (Placeholder)",
      "Strength / differentiator #2 (Placeholder)",
      "Preferred role / team style #3 (Placeholder)",
    ],
  },
  about:
    "Short about paragraph (3-5 sentences max). Explain what you do, what you enjoy building, and the kinds of problems you like solving. Keep it recruiter-friendly. (Placeholder)",
  strengths: [
    "Strength / superpower #1 (Placeholder)",
    "Strength / superpower #2 (Placeholder)",
    "Strength / superpower #3 (Placeholder)",
  ],
  skills: {
    frontend: ["Skill (Placeholder)", "Skill (Placeholder)", "Skill (Placeholder)"],
    backend: ["Skill (Placeholder)", "Skill (Placeholder)", "Skill (Placeholder)"],
    tools: [
      "Tool/Practice (Placeholder)",
      "Tool/Practice (Placeholder)",
      "Tool/Practice (Placeholder)",
    ],
  },
  experience: [
    {
      company: "Strandum HR",
      role: "Role Title (Placeholder)",
      dates: "Month 20XX - Month 20XX (Placeholder)",
      bullets: [
        "Responsibility / impact statement #1 (Placeholder)",
        "Responsibility / impact statement #2 (Placeholder)",
        "Tech / collaboration / outcome #3 (Placeholder)",
      ],
    },
  ],
  contact: {
    email: "you@example.com",
    location: "Location (Placeholder)",
    timezone: "Timezone (Placeholder)",
  },
  links: {
    cv: "https://example.com/your-cv.pdf",
    github: "https://github.com/your-handle",
    linkedin: "https://www.linkedin.com/in/your-handle",
  },
  footer: {
    copyright: "All rights reserved. (Placeholder)",
    note: "Built with React + Vite. Content placeholders are editable in",
    notePath: "src/data",
  },
}
